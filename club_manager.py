...
# ==============================================================================
# DATABASE SETUP (SQLite) & AUTO-MIGRATION
# ==============================================================================
conn = sqlite3.connect('club_database.db', check_same_thread=False)
conn.row_factory = sqlite3.Row
c = conn.cursor()

# Create Tables
c.execute('''CREATE TABLE IF NOT EXISTS club_settings (key TEXT PRIMARY KEY, value TEXT)''')
c.execute('''CREATE TABLE IF NOT EXISTS specialties (id TEXT PRIMARY KEY, name TEXT)''')
c.execute('''CREATE TABLE IF NOT EXISTS students (id TEXT PRIMARY KEY, name TEXT, fatherName TEXT, birthDate TEXT, phone TEXT, nationalId TEXT, address TEXT, coachId TEXT, specialty TEXT, status TEXT, insuranceExpiry TEXT, tuitionExpiry TEXT, joinDate TEXT)''')
c.execute('''CREATE TABLE IF NOT EXISTS coaches (id TEXT PRIMARY KEY, name TEXT, phone TEXT, specialty TEXT, nationalId TEXT, salaryType TEXT, salaryValue REAL, status TEXT, joinDate TEXT, logo TEXT)''')
c.execute('''CREATE TABLE IF NOT EXISTS tuitions (id TEXT PRIMARY KEY, studentId TEXT, amount REAL, payDate TEXT, expiryDate TEXT, paymentMethod TEXT, notes TEXT)''')
c.execute('''CREATE TABLE IF NOT EXISTS events (id TEXT PRIMARY KEY, title TEXT, date TEXT, time TEXT, location TEXT, type TEXT)''')
c.execute('''CREATE TABLE IF NOT EXISTS attendance (id TEXT PRIMARY KEY, date TEXT, specialty TEXT, presentStudentIds TEXT)''')
c.execute('''CREATE TABLE IF NOT EXISTS sms_logs (id TEXT PRIMARY KEY, recipientName TEXT, recipientPhone TEXT, messageText TEXT, date TEXT)''')

# Auto-Migrate DB (آپدیت خودکار دیتابیس قدیمی برای اضافه کردن ستون‌های جدید)
try:
    c.execute("ALTER TABLE coaches ADD COLUMN schedule TEXT")
...
try:
    c.execute("ALTER TABLE coaches ADD COLUMN username TEXT")
except sqlite3.OperationalError: pass

try:
    c.execute("ALTER TABLE coaches ADD COLUMN password TEXT")
except sqlite3.OperationalError: pass

conn.commit()

# Default Specialties setup if empty
c.execute("SELECT count(*) FROM specialties")
if c.fetchone()[0] == 0:
    c.executemany("INSERT INTO specialties VALUES (?, ?)", [('sp-1', 'هاپکیدو'), ('sp-2', 'تکواندو'), ('sp-3', 'کاراته'), ('sp-4', 'دفاع شخصی')])
    conn.commit()
...
if 'db_loaded' not in st.session_state:
    c.execute("SELECT value FROM club_settings WHERE key='club_name'")
    res_name = c.fetchone()
    st.session_state.club_name = res_name['value'] if res_name else "باشگاه هاپکیدو لیام"
    
    c.execute("SELECT value FROM club_settings WHERE key='club_logo'")
    res_logo = c.fetchone()
    st.session_state.club_logo = res_logo['value'] if res_logo else ""
    
    c.execute("SELECT value FROM club_settings WHERE key='admin_username'")
    res_user = c.fetchone()
    st.session_state.admin_username = res_user['value'] if res_user else "admin"
    
    c.execute("SELECT value FROM club_settings WHERE key='admin_password'")
    res_pass = c.fetchone()
    st.session_state.admin_password = res_pass['value'] if res_pass else "1234"

    for table in ['specialties', 'students', 'coaches', 'tuitions', 'events', 'sms_logs']:
        c.execute(f"SELECT * FROM {table}")
        st.session_state[table] = [dict(row) for row in c.fetchall()]
    
    c.execute("SELECT * FROM attendance")
    st.session_state.attendance = []
    for row in c.fetchall():
        r_dict = dict(row)
        r_dict['presentStudentIds'] = json.loads(r_dict['presentStudentIds'])
        st.session_state.attendance.append(r_dict)
...
if view_mode == "📝 فرم ثبت‌نام عمومی (لینک وب)":
    ...
    if st.session_state.coaches:
        # ایجاد شبکه موزاییکی (دو ستون در هر ردیف)
    ...
    with st.form("public_registration", clear_on_submit=True):
        ...
        if submitted:
            if not name or not phone or not national_id:
                ...
                c.execute("INSERT INTO students VALUES (:id, :name, :fatherName, :birthDate, :phone, :nationalId, :address, :coachId, :specialty, :status, :insuranceExpiry, :tuitionExpiry, :joinDate, :photo)", new_s)
                conn.commit()
...
# ==============================================================================
    ...
    if not st.session_state.logged_in:
        ...
        menu_options = {
            ...
            "tuition": "💳 سیتیوانسیون خزانه‌داری",
            "sms": "💬 سامانه پیامک",
            "events": "📅 رویدادها",
            "settings": "⚙️ تنظیمات"
        }
                    ...
                    with st.form("edit_student_form"):
                        ...
                        with ecol2:
                            ...
                            if cur_st.get('coachId') and any(c['id'] == cur_st['coachId'] for c in c_opts):
                                c_idx = [c['id'] for c in c_opts].index(cur_st['coachId']) + 1
                            e_coach = st.selectbox("مربی:", options=[""] + [c['id'] for c in c_opts], index=c_idx, format_func=lambda x: next((c['name'] for c in c_opts if c['id'] == x), "--- انتخاب کنید ---")) if c_opts else ""
                            
                            s_opts = active_specialties_names if active_specialties_names else ["نامشخص"]
                            s_idx = s_opts.index(cur_st['specialty']) if cur_st.get('specialty') in s_opts else 0
                            e_spec = st.selectbox("رشته ورزشی:", options=s_opts, index=s_idx)
                        ...
                        with sub_edit:
                            if st.form_submit_button("💾 ذخیره تغییرات", type="primary"):
                                ...
                                st.session_state.students[st_idx].update({
                                ...
                                c.execute('''UPDATE students SET name=?, fatherName=?, nationalId=?, birthDate=?, phone=?, coachId=?, specialty=?, address=?, insuranceExpiry=?, tuitionExpiry=?, status=?, photo=? WHERE id=?''', 
                                          (e_name, e_father, e_nid, e_birth, e_phone, e_coach, e_spec, e_address, e_ins, e_tui, e_status, final_photo, cur_st['id']))
                                conn.commit()
                        ...
                        with sub_del:
                            if st.form_submit_button("❌ حذف هنرجو"):
                                c.execute("DELETE FROM students WHERE id=?", (cur_st['id'],))
                                conn.commit()
...
<div style="margin-top: 20px; font-size: 11px; border-top: 1px dashed rgba(255,255,255,0.2); padding-top: 15px; display:flex; justify-content:space-between;">
    ...
    <div style="text-align:left;">
                    ...
                    with st.form("edit_coach_form"):
                        ...
                        with sub_edit:
                            if st.form_submit_button("💾 ذخیره تغییرات", type="primary"):
                                ...
                                st.session_state.coaches[c_idx].update({
                                ...
                                c.execute('''UPDATE coaches SET name=?, phone=?, specialty=?, schedule=?, logo=?, username=?, password=? WHERE id=?''', 
                                          (e_c_name, e_c_phone, e_c_spec, e_c_schedule, final_logo, e_c_username, e_c_password, cur_coach['id']))
                                conn.commit()
                        ...
                        with sub_del:
                            if st.form_submit_button("❌ حذف مربی"):
                                c.execute("DELETE FROM coaches WHERE id=?", (cur_coach['id'],))
                                conn.commit()
            ...
            with tab_add:
                with st.form("add_coach_form", clear_on_submit=True):
                    ...
                    if st.form_submit_button("ثبت مربی"):
                        ...
                        c.execute("INSERT INTO coaches VALUES (:id, :name, :phone, :specialty, :nationalId, :salaryType, :salaryValue, :status, :joinDate, :logo, :schedule, :username, :password)", new_c)
                        conn.commit()
                    ...
                    if not class_students:
                        ...
                        if st.button("💾 ذخیره لیست حاضرین", type="primary"):
                            if exist_rec:
                                ...
                                c.execute("UPDATE attendance SET presentStudentIds=? WHERE id=?", (json.dumps(present_list), exist_rec['id']))
                            else:
                                new_att = {'id': f'att-{int(time.time())}', 'date': selected_date, 'specialty': selected_spec, 'presentStudentIds': present_list}
                                c.execute("INSERT INTO attendance VALUES (?, ?, ?, ?)", (new_att['id'], new_att['date'], new_att['specialty'], json.dumps(new_att['presentStudentIds'])))
        ...
        # ----------------------------------------------------------------------
            ...
            st.title("💳 سیتیوانسیون خزانه‌داری")
                ...
                with st.form("pay_form", clear_on_submit=True):
                    ...
                    if active_st:
                        ...
                        if st.form_submit_button("ثبت فیش و تمدید"):
                            ...
                            c.execute("INSERT INTO tuitions VALUES (:id, :studentId, :amount, :payDate, :expiryDate, :paymentMethod, :notes)", new_t)
                            c.execute("UPDATE students SET tuitionExpiry=? WHERE id=?", (pay_expiry, pay_st_id))
                            conn.commit()
        ...
        # ----------------------------------------------------------------------
            ...
            with col_send:
                ...
                custom_msg = template_text.replace("[نام]", st_info['name']).replace("[تاریخ]", to_persian_digits(st_info['tuitionExpiry']))
                ...
                if st.button("✉️ ارسال پیامک", type="primary"):
                    ...
                    c.execute("INSERT INTO sms_logs VALUES (:id, :recipientName, :recipientPhone, :messageText, :date)", new_log)
                    conn.commit()
        ...
        # ----------------------------------------------------------------------
            ...
            with st.form("event_form", clear_on_submit=True):
                ...
                if st.form_submit_button("ثبت رویداد"):
                    ...
                    c.execute("INSERT INTO events VALUES (:id, :title, :date, :time, :location, :type)", new_ev)
                    conn.commit()
                ...
                with st.form("logo_form"):
                    ...
                    if st.form_submit_button("ذخیره لوگو"):
                        if club_logo_file:
                            ...
                            c.execute("REPLACE INTO club_settings (key, value) VALUES ('club_logo', ?)", (b64_logo,))
                            conn.commit()
            ...
            with col2:
                ...
                with st.form("name_form"):
                    ...
                    if st.form_submit_button("ذخیره نام"):
                        ...
                        c.execute("REPLACE INTO club_settings (key, value) VALUES ('club_name', ?)", (new_club_name,))
                        conn.commit()
                        st.rerun()
            ...
            with st.form("admin_credentials_form"):
                ...
                if st.form_submit_button("ذخیره اطلاعات ورود"):
                    ...
                    c.execute("REPLACE INTO club_settings (key, value) VALUES ('admin_username', ?)", (new_user,))
                    c.execute("REPLACE INTO club_settings (key, value) VALUES ('admin_password', ?)", (new_pass,))
                    conn.commit()
